...
// NEW: Use fetchWithAuth for student results
export const fetchStudentResults = async () => {
    return await fetchWithAuth('/student/results');
};
...
export const uploadTopicPdf = async (topicId, file) => {
  ...
  return await fetchWithAuth(`/topics/${topicId}/upload-pdf`, 'PUT', formData);
};
...
export const getUserTimetable = async () => {
    ...
    if (!user || !user.level) {
        throw new Error("User level not found in localStorage");
    }
    // Corrected to use fetchWithAuth
    return await fetchWithAuth(`/timetable/${user.level}`);
};

export const createTimetable = async (data) => {
    // Corrected to use fetchWithAuth
    return await fetchWithAuth('/timetable', 'POST', data);
};
...
export const updateTimetable = async (id, data) => {
    // Corrected to use fetchWithAuth
    return await fetchWithAuth(`/timetable/${id}`, 'PUT', data);
};
...
// This route is NOT in the admin router. It's likely in the users.router as /users/me
// You mentioned /academy/api/me/ is working (200 OK), so this is probably fine as is.
export const getMyStudentProfile = async () => {
  // This likely calls /academy/api/users/me, which is correct
  return await fetchWithAuth(`/users/me`);
};

// 🔍 Admin: Get all student profiles
export const getAllStudentProfiles = async () => {
  // FIX: Add '/admin' to the path
  return await fetchWithAuth(`/admin/student_profiles`);
};

// ➕ Admin: Create a new student profile
export const createStudentProfile = async (data) => {
  // FIX: Add '/admin' to the path
  return await fetchWithAuth(`/admin/student_profiles`, 'POST', data);
};

// 🛠️ Admin: Update a student profile by user ID
export const updateStudentProfile = async (userId, data) => {
  // FIX: Add '/admin' to the path
  return await fetchWithAuth(`/admin/student_profiles/${userId}`, 'PUT', data);
};

// These next two functions seem to be for a student updating their own profile.
// The backend `student_profile.py` has routes for `/me` and `/me/image` on a router with an empty prefix.
// The `main.py` shows the `student_profile.router` is prefixed with `/admin/student_profiles`.
// This means the student-facing routes are also under `/admin`. This seems like a design flaw.
// Correcting the calls to match the backend config:

// Student: Update my own profile
export const updateMyStudentProfile = async (data) => {
  // FIX: Add '/admin' to the path
  return await fetchWithAuth('/admin/student_profiles/me', 'PUT', data);
};

// Student: Upload my own profile image
export const uploadProfileImage = async (formData) => {
  // FIX: Add '/admin' to the path
  return await fetchWithAuth('/admin/student_profiles/me/image', 'PUT', formData);
};

// 🔍 Admin: Get a student profile by user ID (admin use)
// Your router has a GET route at `/`, but not a GET route at `/{user_id}`
// This function needs to be checked against the backend.
// The backend has `@router.get("/")` for listing all profiles, and `@router.put("/{user_id}")` for updating.
// There is no `GET` for a single profile by user_id in the provided code.
// The correct endpoint might be something you've omitted, but if it exists, the call should be:
export const getStudentProfileByUserId = async (userId) => {
  // Assuming a route like `/admin/student_profiles/{user_id}` exists on the backend
  return await fetchWithAuth(`/admin/student_profiles/${userId}`);
};
...
export const getAllTeachers = async () => {
    return await fetchWithAuth('/admin/teachers');
};

export const assignSubjectToTeacher = async (teacherId, subjectId) => {
    const data = { teacher_id: teacherId, subject_id: subjectId };
    return await fetchWithAuth('/admin/assign-subject', 'POST', data);
};
...
export const fetchTeachersWithSubjects = async () => {
    return await fetchWithAuth('/admin/teachers-with-subjects');
};
...
export const fetchTeacherSubjects = async (studentClass, level, teacherId) => {
    ...
    return await fetchWithAuth(`/subjects/teacher?${params}`);
};
...
export const regenerateQuestions = async (topicId, qtype) => {
    return await fetchWithAuth(`/topics/${topicId}/generate-questions?qtype=${qtype}`, 'POST');
};
