...
function AdminStudentProfiles() {
  ...
  const handleSave = async (userId) => {
    try {
      ...
      // NOTE: This code uses 'alert', which can be disruptive. Consider using a custom modal or notification system for a better user experience.
      alert('Update failed');
    }
  };
        ...
        <table className="min-w-[1400px] w-full text-sm">
          ...
          <tbody className="bg-white dark:bg-gray-800">
            {profiles.map((profile) => {
              ...
              // Construct the URL using the base URL and the correct static path from the backend
              const normalizedImageUrl = profile.profile_image
                ? `${BASE_URL}static/profiles/${profile.profile_image.split('/').pop()}`
                : null;
