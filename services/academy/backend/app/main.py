...
def custom_openapi():
    ...
    openapi_schema["components"]["securitySchemes"] = {
        "OAuth2PasswordBearer": {
            ...
            "flows": {
                "password": {
                    "tokenUrl": f"{API_PREFIX}/token", # Corrected tokenUrl
                    "scopes": {}
                }
            }
        }
...
if not ALLOWED_ORIGINS:
    ALLOWED_ORIGINS = [
        ...
        "http://localhost:8080",
        "http://127.0.0.1:8080",
        "https://020f77fa8425.ngrok-free.app/"
    ]
...
# FIX: The path to mount the static files must include the API_PREFIX
# This makes the server look for files at the same path the frontend is requesting.
app.mount(f"{API_PREFIX}/static/profiles", StaticFiles(directory=str(PROFILE_IMAGE_DIR)), name="profile_images")
...
app.include_router(zoom_classes.router, prefix=f"{API_PREFIX}", tags=["Zoom Classes"])
app.include_router(answers.router, prefix=f"{API_PREFIX}/answers", tags=["Answers"])
app.include_router(topic_questions.router, prefix=f"{API_PREFIX}/topics", tags=["Topic Questions"])
app.include_router(timetable.router, prefix=f"{API_PREFIX}/timetable", tags=["Timetable"])
app.include_router(student_profile.router, prefix=f"{API_PREFIX}/admin/student_profiles", tags=["Student Profile"])
app.include_router(attendance.router, prefix=f"{API_PREFIX}/attendance", tags=["Attendance"])
app.include_router(test_exam.router, prefix=f"{API_PREFIX}/exams", tags=["Exams"])
app.include_router(report_generator.router, prefix=f"{API_PREFIX}/report-cards", tags=["Reports"])
app.include_router(personalization.router, prefix=f"{API_PREFIX}/personalization", tags=["Personalization"])
app.include_router(teachers.router, prefix=f"{API_PREFIX}/teacher", tags=["Teachers"])
app.include_router(parents.router, prefix=f"{API_PREFIX}/parents", tags=["Parents"])
app.include_router(auth_router.router, prefix=f"{API_PREFIX}", tags=["Auth"])
app.include_router(quizzes.router, prefix=f"{API_PREFIX}/student/quizzes", tags=["Quizzes"])
...
app.include_router(admin.admin_router, prefix=f"{API_PREFIX}/admin", tags=["Admin"])
app.include_router(admin_progress.router, prefix=f"{API_PREFIX}/admin/progress", tags=["Admin Progress"])
app.include_router(users.router, prefix=f"{API_PREFIX}/users", tags=["Users"])
app.include_router(chat_router.router, prefix=f"{API_PREFIX}/chat", tags=["Chat"])
app.include_router(student_progress.router, prefix=f"{API_PREFIX}/student", tags=["Student Progress"])
app.include_router(assignment_routes.router, prefix=f"{API_PREFIX}/assignments", tags=["Assignments"])
app.include_router(messaging_router, prefix=f"{API_PREFIX}", tags=["Messaging"])
app.include_router(admin_dashboard_router.router, prefix=f"{API_PREFIX}/admin", tags=["Admin Dashboard"])
app.include_router(admin_activity.router, prefix=f"{API_PREFIX}/admin-activity", tags=["Admin Activity"])
app.include_router(ask_me_anything.router, prefix=f"{API_PREFIX}/ask", tags=["Ask Me Anything"])
app.include_router(parent_dashboard_router.router, prefix=f"{API_PREFIX}/parent-dashboard", tags=["Parent Dashboard"])

# -------------------- Auth Routes --------------------

@app.post(f"{API_PREFIX}/register/", response_model=schemas.UserOut)
def register(
    ...
    new_user = models.User(
        ...
        student_shift=user.student_shift, # <-- Add this line
...
@app.get(f"{API_PREFIX}/me/", response_model=schemas.UserOut)
def read_users_me(current_user: models.User = Depends(get_current_user)):
    return current_user

@app.get(f"{API_PREFIX}/admin-only")
...
@app.get(f"{API_PREFIX}/students/", response_model=List[schemas.UserOut])
...
@app.put(f"{API_PREFIX}/students/{{student_id}}", response_model=schemas.UserOut)
...
@app.post(f"{API_PREFIX}/generate-questions/{{pdf_id}}", response_model=List[schemas.QuestionOut])
...
@app.post(f"{API_PREFIX}/answer/", response_model=schemas.UserAnswerOut)
...
@app.get(f"{API_PREFIX}/questions/by-pdf/{{pdf_id}}", response_model=List[QuestionOut])
...
@app.get(f"{API_PREFIX}/test-llama/")
def test_llama():
...
@app.get(f"{API_PREFIX}/test-connection")
def test_connection():
