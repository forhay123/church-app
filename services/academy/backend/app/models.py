...
class User(Base):
    ...
    zoom_meetings: Mapped[list["ZoomMeeting"]] = relationship("ZoomMeeting", back_populates="host")
...
class Timetable(Base):
    ...
    zoom_meeting = relationship("ZoomMeeting", back_populates="timetable_slot", uselist=False)
...
class ZoomMeeting(Base):
    ...
    timetable_id: Mapped[int] = mapped_column(Integer, ForeignKey("timetables.id"), unique=True, nullable=True)

    host: Mapped["User"] = relationship("User", back_populates="zoom_meetings")
    timetable_slot = relationship("Timetable", back_populates="zoom_meeting")

    def __repr__(self):
        return f"<ZoomMeeting(id={self.id}, topic='{self.topic}', timetable_id={self.timetable_id})>"
